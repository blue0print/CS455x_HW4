...
public class Julia {
    ...
    public Julia(int xRes, int yRes, double cxMin, double cxMax, double cyMin, double cyMax) {
        ...
        for(int y=0; y<yRes; y++){
            for(int x=0; x<xRes; x++){
                double yD = y;
                double xD = x;
                zReal = zMin.real + xD / (xRes - 1.0) * (zMax.real - zMin.real);
                zImag = zMin.imag + yD / (yRes - 1.0) * (zMax.imag - zMin.imag);
                ComplexNumber z = new ComplexNumber(zReal,zImag);
                //ComplexNumber z = new ComplexNumber(0,0);

                int maxIter=200, iter=0, val=0;
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        Julia juli = new Julia(640,480,CXMIN,CXMAX,CYMIN,CYMAX);
        juli.displayFractal(g1,colors);
        juli.saveFractal(juli.filename,colors);
